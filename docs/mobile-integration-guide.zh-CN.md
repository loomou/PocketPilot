...
认证、工作区与 Claude session 发现、历史记录、实时会话传输、任务控制、审批、
...
- PocketPilot 是已安装 Claude Code/Agent SDK 外层的认证传输和运行时控制器，
  不定义第二套会话协议。
- `/v1/tasks/{taskId}/sdk` 是双向原始 SDK JSON。客户端帧是
  `SDKUserMessage`，服务端帧是 `SDKMessage`。
- `/v1/events` 只承载 PocketPilot 控制事件。即使包装在 `kind: "sdk"` 中，
  也绝不能通过该通道传输 SDK 消息。
- 历史记录行是 SDK `SessionMessage` 对象。它们不是实时 `SDKMessage`，
  也不能重新转换为 prompt。
...
| SDK `uuid` | SDK 历史/实时消息身份及 SDK 回放锚点 | 有值时用于历史/实时去重和 `afterUuid`。合法实时消息可以没有该字段。 |
...
| `/v1/tasks/{taskId}/sdk` | 双向 WebSocket | 握手时使用 Bearer header | 移动端向 Agent 发送原始 `SDKUserMessage`；Agent 向移动端发送原始 `SDKMessage` |
...
1. 完成配对并保留设备密钥和轮换凭据。
2. 完成认证并调用 `GET /v1/workspaces`。
3. 选择一个已授权工作区，再调用 `GET /v1/sessions`。
4. 选择一个 Claude session，或选择新建对话。
5. 对已有 session，加载最新一页历史记录用于展示。
6. attach 所选 session 或创建对话运行时。在内部把返回的 `task.id` 保存为
   `taskId`；不要向用户展示单独的 task 步骤。
7. 打开 `/v1/events` 并订阅 `taskId`。
8. 打开 `/v1/tasks/{taskId}/sdk`。服务端会先安装原始订阅者，再激活新建或恢复的
   Query。
9. 原始 socket 打开后，请求 composer options，并用原始 `system/init` 和
   `system/status` 消息校准状态。
10. 将普通 prompt 和 slash command 作为原始 `SDKUserMessage` 帧发送。
...
| Sessions | `GET /v1/sessions` | `listClaudeSessions` | 受保护；列出工作区 sessions |
| Sessions | `POST /v1/sessions` | `createClaudeConversation` | 受保护；创建空历史运行时 |
| Sessions | `GET /v1/sessions/{sessionId}/messages` | `readClaudeSessionHistory` | 受保护；读取按时间正序排列的历史页 |
| Sessions | `POST /v1/sessions/{sessionId}/attach` | `attachClaudeSession` | 受保护；attach/复用 session 运行时 |
...
| SDK | `GET /v1/tasks/{taskId}/sdk` | `streamTaskSdkMessages` | 受保护 WebSocket upgrade |
...
响应为 `{ sessions: SDKSessionInfo[], nextOffset: number | null }`。数据行归 SDK
所有，未来可能增加字段。只要 `nextOffset` 非 null 就应继续，即使某一页经过
workspace policy 过滤后 `sessions` 数组为空。Worktree 不包含在结果中；
...
并发送到 `POST /v1/sessions`。Claude Code 自行解析模型、权限和推理强度默认值。
...
每页最多 50 条消息。`hasMoreBefore` 为 true 时，把返回的 `page.beforeUuid`
传给下一次请求，加载更早一页并 prepend 到现有列表。在同一条 cursor 链上必须保持
`includeSystemMessages` 不变。收到 `HISTORY_CURSOR_STALE` 时，丢弃当前分页链，
不带 `beforeUuid` 重新加载最新页。
...
对 `POST /v1/sessions`，响应 action 为 `created`，`sdkSessionId` 初始值为
...
重连时可追加 `?afterUuid=<lastSdkUuid>`。WebSocket route 会先订阅原始消息，再激活
...
- 已知 `afterUuid` 会回放其后的已保留消息，然后继续实时传输。
- 缺少或无法识别的 `afterUuid` 会从当前 turn 起点回放已保留消息。按 UUID 去重。
- 回放只保留活动 turn，并在 turn 结束、terminal 清理、shutdown 和安全启动时删除。
  在 idle 状态重连可能没有可回放数据。
- 没有 UUID 的消息也会投递和保留，但不能作为 `afterUuid` 锚点。
...
如果不存在活动 Query/runtime，应在所选 workspace 使用 `POST /v1/sessions`
...
| 原始 SDK | `4011 SDK_TRANSPORT_FAILED` | Backoff，刷新 task 状态，使用最后的 `afterUuid` 重连；绝不能自动重发 prompt |
...
sequenceDiagram
    ...
    M->>S: 连接 /tasks/{taskId}/sdk?afterUuid
    S->>S: 安装原始 subscriber
...
sequenceDiagram
    ...
    participant S as 原始 SDK WS
    M->>R: GET messages（无 beforeUuid，limit 50）
    R-->>M: 最新正序页和 beforeUuid
