...
> `@anthropic-ai/claude-agent-sdk@0.3.210`；验证日期为 2026-07-21。
>
> **Schema 来源：** 运行中 Agent 的本地 Swagger UI
> `/documentation/`、原始 OpenAPI `/documentation/json`、随包发布的
> [`dist/openapi/mobile-v1.json`](../dist/openapi/mobile-v1.json)、已安装的
> Claude Agent SDK 类型声明，以及 provider capability 快照。本指南解释多
> provider 工作流和数据所有权；字段定义仍以 OpenAPI 与 provider-native 类型为准。

## 指南地图

1. [共享认证、工作区与任务模型](#1-目的受众版本与事实来源)
2. [多 provider 发现与 readiness](#3-传输概览与端到端流程)
3. [共享 REST 清单](#33-rest-操作清单)
4. [Agent WebSocket 与控制事件 WebSocket](#2-不可破坏的边界与身份模型)
5. [Provider 差异矩阵](#18-provider-差异矩阵)
6. [基于 capability 的客户端清单](#19-基于-capability-的客户端清单)
7. [错误恢复](#14-错误与-close-code-恢复)
8. [Provider 专用指南](#20-provider-专用指南)

## 1. 目的、受众、版本与事实来源

本指南适用于连接用户自有 PocketPilot Agent 的任何移动端客户端。内容包括配对、
认证、多 provider 发现/readiness、工作区与会话发现、历史记录、实时会话传输、
共享任务控制、审批、重连、固定 Claude Command 面板，以及同工作区新建对话行为。
本指南不绑定任何平台，并面向多 provider：Claude 与 Codex 共享认证、工作区、
task 与控制事件；每个 provider 拥有各自的原生 Agent 帧与会话行。

当不同来源存在差异时，按以下优先级判断：

1. REST 和 PocketPilot 控制消息以当前运行时 Zod schema 和生成的 OpenAPI 为准。
2. 封闭的 `historyFilters`、`nativeActions`、`statusCatalogs`、
   `threadManagement` 以 provider capability 快照为准。
3. 原始 Claude `SDKUserMessage`、`SDKMessage`、`SessionMessage` 和
   `PermissionResult` 以 `@anthropic-ai/claude-agent-sdk@0.3.210` 类型声明为准。
4. Codex App Server 方法与重连规则以 provider 专用指南为准。
5. 跨接口调用顺序和客户端状态所有权以本指南为准。

英文文件是规范源。
[`简体中文指南`](./mobile-integration-guide.zh-CN.md) 是它的严格翻译。即使 JSON
仍可解析，也不能直接认定后端或 SDK 升级兼容：移动端发布前必须根据 OpenAPI
重新生成 REST 绑定、复核固定 provider 契约，并重新执行第 16 节的流程。
...
| --- | --- | --- |
| `providerId` | 稳定的 provider path 标识，例如 `claude` 或 `codex` | 从 `/v1/providers` 发现；不能通过本机文件推断支持情况，也不能写死可用状态。 |
| `conversationId` | provider-scoped REST path 使用的 provider-native 会话标识 | Claude：未修改 `SDKSessionInfo` 行中的 SDK session ID。Codex：原生 `threadId`。 |
| `taskId` | PocketPilot 运行时/控制句柄，用于路由、状态、审批、回放、调度和一个存活 provider 流 | 跨 turn 与 Claude `conversation_reset` 保持不变。不能作为面向用户的对话身份。 |
| `sdkSessionId` / 原始 `session_id` | Claude 持久化与恢复身份 | 读取 SDK 提供的值。不能从 `taskId` 推导，也不能用重放历史记录替代。 |
| `nativeConversationId` / Codex `threadId` | Codex 会话分支 | 与 `taskId` 不同。Codex 的 REST `conversationId` 就是它。 |
| `nativeSessionId` | Codex 会话树根身份 | fork 可能共享；不能当作 `taskId`。 |
| SDK `uuid` | Claude 历史/实时消息身份及 Claude 回放锚点 | 有值时用于历史/实时去重和 Claude `afterCursor` 中的 provider-native 游标。合法实时消息可以没有该字段。 |
| Codex transport `afterCursor` | Codex Agent 流的 PocketPilot 回放位置 | 不透明；不是 thread/turn/item ID。Codex 还会发送订阅时 `agent.checkpoint`。 |
| `new_conversation_id` | 原始 Claude `conversation_reset` 发出的新 transcript 边界 | 挂载全新 transcript 并清除缓存标题。不要更换 `taskId`，也不要假定它等于 `session_id`。 |
| `operationId` | 客户端为幂等 HTTP mutation 生成的 UUID | 每个不同 mutation 生成一次；仅重试同一 mutation 时复用。绝不能放入 provider-native 帧。 |
| 审批 `requestId` | provider-native 权限回调身份 | Claude：经 REST 解决的 SDK 回调 ID。Codex：在 Agent socket 上回答的原生 JSON-RPC 请求 ID。与 HTTP `operationId` 无关。 |
| 控制 `cursor` | PocketPilot 控制事件回放位置 | 按 `taskId` 保存，并作为 `/v1/events` 的 `afterCursor` 发送。与 SDK `uuid` 或 Codex transport cursor 无关。 |
...
### 3.2 标准多 provider 用户流程

1. 完成配对并保留设备密钥和轮换凭据。
2. 完成认证，调用 `GET /v1/providers`，并选择状态为 `available` 的 provider。
   不可用 provider 仍会列出，并带稳定 `reasonCode`（例如
   `CODEX_COMMAND_NOT_FOUND`、`CODEX_APP_SERVER_VERSION_UNSUPPORTED`、
   `CODEX_APP_SERVER_PROBE_FAILED`、`CLAUDE_SDK_NOT_AVAILABLE`、
   `CLAUDE_SDK_VERSION_UNSUPPORTED` 或 `CLAUDE_SDK_PROBE_FAILED`）。discovery 与
   capabilities 会按约 30s 的服务端 TTL 刷新主机 readiness，且从不暴露安装路径、
   凭据或原始进程诊断。
3. 读取 `/v1/providers/{providerId}/capabilities`，在渲染 provider 专用控件前保存
   封闭 capability 对象。
4. 调用 `GET /v1/workspaces`，让用户选择完全相同的已授权根目录。
5. 用 `GET /v1/providers/{providerId}/conversations` 列会话。若已发布，可使用可选
   `includeArchived` / `searchTerm` 过滤。
6. 选择 provider-native 会话，或选择新建对话。
7. 对已有会话，加载最新一页历史记录用于展示。遵守
   `historyFilters.includeSystemMessages`；绝不要发送不支持的过滤。
8. attach 所选会话或创建会话运行时。在内部把返回的 `task.id` 保存为
   `taskId`；不要向用户展示单独的 task 步骤。
9. 打开 `/v1/events` 并订阅 `taskId`。
10. 打开 `/v1/tasks/{taskId}/agent`。服务端会在激活前安装 native 订阅者。对
    Codex，首帧可能是 `agent.checkpoint`。
11. 按 `task.provider` 分支：
    - Claude：请求 composer options，并发送原始 `SDKUserMessage` 帧。
    - Codex：只发送白名单原生 App Server 方法；动作、目录、审批与重连见 Codex
      移动端指南。

task 创建、选择和恢复都是内部传输细节，不应变成额外的会话页面。用户选择
provider-native 会话本身就是继续该会话的操作。
...
| Providers | `GET /v1/providers` | `listAgentProviders` | 受保护；列出本机可用和不可用 provider，并刷新 readiness |
| Providers | `GET /v1/providers/{providerId}/capabilities` | `getAgentProviderCapabilities` | 受保护；读取状态、native protocol version 和封闭 capabilities |
| Conversations | `GET /v1/providers/{providerId}/conversations` | `listAgentConversations` | 受保护；列出工作区内的 provider-native 会话；在已发布时支持可选 `includeArchived` / `searchTerm` |
...
| Conversations | `POST /v1/providers/{providerId}/conversations/{conversationId}/fork` | `forkAgentConversation` | 受保护；当 `threadManagement.fork` 为 true 时 fork；返回绑定到**新**会话的 task |
| Conversations | `POST /v1/providers/{providerId}/conversations/{conversationId}/archive` | `archiveAgentConversation` | 受保护；已发布时 archive；需要 `confirm: true`；返回 null task，且**不会**自动关闭已绑定 task |
| Conversations | `POST /v1/providers/{providerId}/conversations/{conversationId}/unarchive` | `unarchiveAgentConversation` | 受保护；已发布时 unarchive；返回 null task |
| Conversations | `POST /v1/providers/{providerId}/conversations/{conversationId}/delete` | `deleteAgentConversation` | 受保护；已发布时 delete；需要 `confirm: true`；仅在原生成功后把绑定的非 terminal task 置为 terminal |
...
| Composer | `GET /v1/tasks/{taskId}/composer-options` | `getTaskComposerOptions` | 受保护；Claude 模型、模式和推理强度 |
| Composer | `POST /v1/tasks/{taskId}/model` | `setTaskModel` | 受保护 mutation；Claude 下一 turn 的模型 |
| Composer | `POST /v1/tasks/{taskId}/permission-mode` | `setTaskPermissionMode` | 受保护 mutation；Claude 权限模式 |
| Composer | `POST /v1/tasks/{taskId}/effort` | `setTaskEffort` | 受保护 mutation；Claude 下一 turn 的推理强度 |
| 审批 | `POST /v1/tasks/{taskId}/approvals/{requestId}` | `resolveTaskApproval` | 受保护 mutation；提交完整 Claude SDK `PermissionResult` |
...
会话生命周期 mutation 只走这些 REST 路由。不要为 fork、archive、unarchive 或
delete 发明 Agent WebSocket 包装。Codex 当前发布 thread-management 表面；Claude
当前把这些标志发布为 `false`。confirm 语义、原生动作、状态目录与 Codex 重连细节
见 [Codex 移动端指南](./codex-mobile-integration-guide.zh-CN.md)。
...
| `AGENT_PROVIDER_NOT_FOUND` | 404 | 修正客户端路由；重读 `/v1/providers` |
| `AGENT_PROVIDER_UNAVAILABLE` | 409 | provider 已注册但不可执行；刷新 discovery，并引导用户到电脑端 |
...
| `CODEX_THREAD_NOT_FOUND` | 404 | 删除 stale Codex thread 行并重载会话列表 |
| `CODEX_HISTORY_UNAVAILABLE` | 409 | 保留当前 UI；稍后重试历史；绝不要把历史当 prompt 重发 |
...
| `CONFIRMATION_REQUIRED` | 409 | archive/delete 需要显式 `confirm: true`；展示确认 UI 后重试 |
...
| `TASK_CONTROL_NOT_SUPPORTED` | 409 | 对非 Claude task（或不支持的表面）使用了 Claude 专用控制；改用 provider-native 路径 |
...
| `CODEX_REQUEST_NOT_ALLOWED` | 403 | 被拒绝的 Codex 方法/参数；修正客户端载荷并查阅 Codex 白名单 |
| `CODEX_APP_SERVER_UNAVAILABLE` | 503/409 | 展示 provider 不可用；等待主机/App Server 恢复 |
...
| --- | --- | --- |
| Agent | `4000 SDK_MESSAGE_INVALID` | 客户端 bug：检查发出的 provider-native frame；不要原样重发 |
| 两者 | `4003 AUTHENTICATION_FAILED` 或设备撤销 reason | 刷新/验证凭据；撤销后重新配对 |
| Agent | `4004 TASK_NOT_FOUND` | 停止重连 task；重新 attach/create |
| Agent | `4009 TASK_SESSION_UNAVAILABLE` | 刷新 task；除非 terminal，否则等待/重新 attach |
| Agent | `4011 SDK_TRANSPORT_FAILED` | Backoff，刷新 task 状态，使用最后的 provider transport cursor 重连；绝不能自动重发 prompt |

关闭 reason 中的 `SDK_*` 措辞是传输兼容命名。对 Codex task，它描述的是
provider-native transport，不是 Claude SDK 消息。

控制 socket 还会在不关闭连接的情况下发送
`EVENT_SUBSCRIPTION_INVALID` JSON 消息。Agent socket 从不发送 PocketPilot JSON
错误帧。网络/`4011` 重连使用带 jitter 的有界指数 backoff；认证和 terminal error
需要修复状态，不能无限循环重试。

Codex 重连说明：每次成功的 Agent 订阅，Codex 可能在保留原生帧前先发送
`agent.checkpoint`。`cursor` 非 null 时保存它；缺失/未知 cursor 仍会正确回退到
完整保留的活动 turn 窗口。Claude 不发送 `agent.checkpoint`。
...
- Provider capability probe：`GET /v1/providers` 与
  `GET /v1/providers/{providerId}/capabilities`

从 OpenAPI 生成 REST request/response binding，但两个 WebSocket 应根据其
`x-websocket` 描述和 provider wire owner 显式建模。Swagger UI 只展示 WebSocket
frame，不能执行它们。

OpenAPI 发生任何变化时，都要先比较 path、`operationId`、认证、错误和 schema，
再重新生成。provider 发生任何升级时，至少重新检查：

- 封闭 capability 对象（`historyFilters`、`nativeActions`、`statusCatalogs`、
  `threadManagement`、`attachments`）；
- Claude `SDKUserMessage`、`SDKMessage`、`SessionMessage` 和
  `PermissionResult` union；
- 原始 Claude `system/init`、`system/status`、command-output 和
  `conversation_reset` 行为；
- Codex 订阅时 `agent.checkpoint`、白名单方法与状态目录投影规则；
- WebSocket base guard 与全部稳定 close code。

不要在移动端专用 schema 中冻结完整 provider union。可用时通过 capability 与
provider 初始化执行 feature detection；保留未知字段；工作流行为变化时同时更新本
指南的两个语言版本。

## 18. Provider 差异矩阵

| 领域 | 共享 PocketPilot 表面 | Claude | Codex |
| --- | --- | --- | --- |
| 认证 / 工作区 / task ID | 相同 Bearer、配对、`/v1/workspaces`、`taskId` | 相同 | 相同 |
| Discovery | `/v1/providers`、capabilities、readiness TTL、稳定 `reasonCode` | Claude reason codes | Codex reason codes |
| 会话 list/create/history/attach | 通用 provider-scoped REST | SDK session 行 / SessionMessage 历史 | 原生 thread / turn-item 历史 |
| Fork / archive / unarchive / delete | 每个 provider 都有 REST 路由 | `threadManagement.*` 当前为 false | 封闭 true 标志；archive/delete 需要 `confirm: true` |
| Agent WebSocket | `/v1/tasks/{taskId}/agent` | 仅原始 SDK 帧 | 原生 JSON-RPC + 订阅时 `agent.checkpoint` |
| 控制 WebSocket | 仅 `/v1/events` | 相同 | 相同；可投影带 provider 标记的 `approval.requested` |
| 历史过滤 | `historyFilters.includeSystemMessages` | `true`；过滤生效 | `false`；`true` → `HISTORY_FILTER_NOT_SUPPORTED` |
| Composer / model / mode / effort | Claude REST 控制 | 支持 | `TASK_CONTROL_NOT_SUPPORTED`；改用原生目录 |
| 审批 | 可能存在控制投影 | 经审批 REST 提交 `PermissionResult` | 在 Agent socket 上回答原生 JSON-RPC 请求 |
| 原生动作 | 封闭 `nativeActions` | 在已评审前为空 | `review` / `rename` / `compact` |
| 状态目录 | 封闭 `statusCatalogs` | 在已评审前为空 / false | account/rateLimits/skills/hooks/mcpServers |
| 附件 | 布尔 capability | 视 provider | `false` |

在选择编解码前先读 `task.provider` 与 `nativeProtocolVersion`。绝不要从路径、
历史形状或消息 `type` 成员推断 provider。

## 19. 基于 capability 的客户端清单

发布多 provider 移动端前，尽可能按封闭 capability 快照门控 UI 与请求，而不是在
所有路径硬编码 provider ID。

- [ ] 配对、刷新与撤销使用本指南的共享认证。
- [ ] provider 选择器列出带稳定 `reasonCode` 的不可用 provider，且从不展示安装
      路径或原始进程诊断。
- [ ] capability 对象按封闭集合处理：
  - `attachments: boolean`
  - `historyFilters: { includeSystemMessages: boolean }`
  - `nativeActions: { review?, rename?, compact? }` 仅这些
  - `statusCatalogs: { account?, rateLimits?, skills?, hooks?, mcpServers? }` 仅这些
  - `threadManagement: { archive?, delete?, fork?, includeArchived?, search?, unarchive? }` 仅这些
- [ ] 列表过滤（`includeArchived`、`searchTerm`）仅在已发布时出现。
- [ ] 历史从不发送不支持的 `includeSystemMessages=true`。
- [ ] fork/archive/unarchive/delete UI 仅在已发布时出现；archive 与 delete 需要
      显式 `confirm: true`。
- [ ] archive 不假定自动关闭已绑定 task；delete 仅在原生成功后把绑定的非
      terminal task 置为 terminal。
- [ ] Agent 与控制 socket 保持分离；不引入 `{ kind: "sdk", payload }` 包装。
- [ ] Claude 路径：subscribe-before-activation、原始 SDK 帧、依赖 Send 前的
      composer 控制、审批 REST、UUID 历史/实时去重。
- [ ] Codex 路径：处理订阅时 `agent.checkpoint`、之后纯原生帧、白名单方法、原生
      动作、状态目录、原生审批响应，以及 Codex 指南中的 checkpoint/`afterCursor`
      重连规则。
- [ ] 不对 Codex task 调用 Claude 专用 REST 控制。
- [ ] 错误恢复覆盖 `CONFIRMATION_REQUIRED`、`HISTORY_FILTER_NOT_SUPPORTED`、
      provider 不可用，以及稳定 Agent 关闭码 `4000` / `4003` / `4004` / `4009` /
      `4011`。
- [ ] 日志、分析与缓存不含凭据、prompt、工具输入、主机绝对路径或原始进程诊断。

## 20. Provider 专用指南

- [Codex 移动端对接指南（en）](./codex-mobile-integration-guide.en.md)
- [Codex 移动端对接指南（zh-CN）](./codex-mobile-integration-guide.zh-CN.md)
- [Codex App Server 对接](./codex-app-server-integration.en.md)
- [Claude SDK 消息类型（zh-CN）](./claude-sdk-message-types.zh-CN.md)

共享认证、工作区、任务模型、REST 清单与 capability 门控使用本通用指南。confirm
语义、checkpoint 重连、原生动作与状态目录使用 Codex 指南。Claude SDK 指南用于本
工作流文档有意不覆盖的消息类型所有权细节。
