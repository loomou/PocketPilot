  ...
  const extendedOperation: OpenAPIV3.OperationObject<WebSocketExtension> = {
    ...
    "x-websocket": {
      ...
      notes: [
        ...
        'Client and server frames are provider-native objects. Codex alone emits one subscribe-time agent.checkpoint control frame ({ kind: "agent.checkpoint", payload: { provider: "codex", cursor: string|null } }) before retained native replay; subsequent Codex frames and all Claude frames stay unwrapped.',
        ...
        "The optional afterCursor query is interpreted by the selected provider. Claude uses an SDK UUID. Codex clients may store the subscribe-time agent.checkpoint cursor and pass it as afterCursor for incremental retained replay; an absent, invalid, or evicted Codex cursor still rebuilds the complete retained active-turn window.",
        "Codex native retained/live frames never receive a cursor or PocketPilot field. Skip agent.checkpoint on the client codec, apply only native frames to the reducer, and after a completed turn reconcile final items from Codex-native history instead of appending replayed deltas to an old projection.",
